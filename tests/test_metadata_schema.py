...
def test_saved_run_metadata_includes_artifact_version(tmp_path) -> None:
    ...
    assert payload["estimator_name"] == "none"
    assert payload["controller_debug"]["solve_calls"] == 1


def test_metric_summary_json_includes_artifact_version_and_required_columns(
    tmp_path,
) -> None:
    result = TrajectoryResult(
        controller_name="LQR",
...
def test_optimizer_saved_metadata_uses_effective_switch_config(tmp_path) -> None:
    result = TrajectoryResult(
        controller_name="Optimizer",
        estimator_name="ekf",
        scenario_name="local_small_angle",
        suite_name="nominal",
        seed=7,
    ...
    roots = artifact_roots(tmp_path / "artifacts_v3")
    ...
    for controller_key, controller_name in optimizer_cases:
        result.controller_name = controller_name
        _, controller_payload = build_hybrid_controller(controller_key, params)
        paths = save_run_artifacts(
            result,
            scenario,
            params,
        ...
        payload = load_json(roots["base"] / paths["json_path"])
        assert payload["switch_config"]["enter_angle_deg"] == 6.0
        assert payload["switch_config"]["enter_rate"] == 0.35
        assert payload["switch_config"]["enter_hold_time"] == 0.20
        assert payload["switch_config"]["exit_angle_deg"] == 10.0
        assert payload["switch_config"]["exit_hold_time"] == 0.05
